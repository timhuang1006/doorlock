...
function initNightVisionSlider() {
    const slider = document.getElementById('nightVisionSlider');
    const afterElement = document.querySelector('.comparison-after');
    ...
    function updateComparison(value) {
        ...
        afterElement.style.clipPath = `inset(0 ${percentage}% 0 0)`;
        if (sliderHandle) {
...
console.log('%c 進階互動 V3 已載入 ✓ ', 'background: #00d4ff; color: #000; font-size: 12px; padding: 4px;');
...
function switchTab(element, targetId) {
    ...
    // 切換內容顯示
    if (targetId) {
        document.querySelectorAll('.security-content-block').forEach(block => {
        ...
        if (target) {
            target.style.display = 'grid'; // 保持 grid 佈局
            target.classList.add('active');

            // 觸發動畫重播 (可選)
            const animatedElements = target.querySelectorAll('.animate-on-scroll');
